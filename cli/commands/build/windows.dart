...
class WindowsBuildCommand extends Command with BuildCommandCommonSteps {
  ...
  void run() async {
    ...
    final chocoFiles = [
      ...
      join(cwd.path, "choco-struct", "pomelo.nuspec"),
    ];
    ...
    final runnerRCFile = File(join(cwd.path, "windows", "runner", "Runner.rc"));

    runnerRCFile.writeAsStringSync(
      runnerRCFile
          .readAsStringSync()
          .replaceAll("%{{POMELO_VERSION}}%", versionWithoutBuildNumber)
          .replaceAll(
            "%{{POMELO_VERSION_AS_NUMBER}}%",
            [
    ...
    final exePath = join(cwd.path, "dist", "Pomelo-windows-x86_64-setup.exe");
    ...
    final chocoNupkg = File(
      join(cwd.path, "dist", "pomelo.$versionWithoutBuildNumber.nupkg"),
    );

    final distNupkgPath = join(cwd.path, "dist", "Pomelo-windows-x86_64.nupkg");
