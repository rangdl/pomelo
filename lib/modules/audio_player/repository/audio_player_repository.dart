...
class AudioPlayerRepository {
  ...
  Future<AudioPlayerState?> restore() async {
    ...
    final tracks = trackEntities
        .map((e) => Track.fromJson(jsonDecode(e.trackJson) as Map<String, dynamic>))
        .toList();
  ...
  Future<void> persist(AudioPlayerState state) async {
    // 保存状态
    await _db.upsertPlayerState(PlayerStateTableCompanion(
  ...
  Future<void> clear() async {
    await _db.upsertPlayerState(const PlayerStateTableCompanion(
      playing: Value(false),
      loopMode: Value('none'),
