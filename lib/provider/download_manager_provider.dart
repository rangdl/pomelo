...
import 'package:pomelo/components/dialogs/replace_downloaded_dialog.dart';
import 'package:pomelo/extensions/dio.dart';
...
class DownloadManagerNotifier extends Notifier<List<DownloadTask>> {
  final Dio dio;
  DownloadManagerNotifier() : dio = Dio(), super();
  ...
  Future<bool> _shouldReplaceFileOnExist(DownloadTask task) async {
    ...
    try {
      return await showDialog<bool>(
            ...
            builder: (context) => ReplaceDownloadedDialog(track: task.track),
          ) ??
          false;
    } finally {
  ...
  Future<void> _downloadTrack(DownloadTask task) async {
    try {
      ...
      // }
      // final ext = container.getFileExtension();
      const ext = 'mp3';
      final savePath = join(
        downloadLocation,
        ServiceUtils.sanitizeFilename(
          "${track.name} - ${track.artists.map((e) => e.name).join(", ")}.$ext",
        ),
      );
      // final savePath = join(
      //   downloadLocation,
