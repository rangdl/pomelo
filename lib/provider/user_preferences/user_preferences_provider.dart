...
// import 'package:pomelo/models/metadata/market.dart';
...
class UserPreferencesNotifier extends Notifier<PreferencesTableData> {
  ...
  build() {
    ...
    (db.select(
      db.preferencesTable,
    )..where((tbl) => tbl.id.equals(0))).getSingleOrNull().then((result) async {
      if (result == null) {
        await db
  ...
  Future<String> _getDefaultDownloadDirectory() async {
    if (kIsAndroid) return "/storage/emulated/0/Download/Pomelo";
