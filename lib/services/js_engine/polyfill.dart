    ...
    return new Promise(function(resolve,reject){
      ...
      if (!options.body){
        if (options.form || options.formData){
          try{
            options.body=JSON.stringify(options.form || options.formData)
          }catch(e){
            options.body=String(options.form || options.formData)
          }
          delete options.form;
      ...
      __native_fetch__(url, {...options,method:method,headers:reqHeaders}).then((res)=>{
        var body=res.data, bodyStr='';
      ...
      }).catch(err => reject(new Error('fetch error: '+(err.message||String(err)))));
    })
  }
}
